...
export class DiceSelector {
  ...
  computerSelectsFirst(): number {
    ...
    console.log(
      `I make the first move and choose the [${this.selectedByComputer.faces.join(
        ","
      )}] dice.`
    );
    return randomIndex;
  ...
  userSelectsFirst(): number {
    ...
    if (
      isNaN(selectedIndex) ||
      selectedIndex < 0 ||
      selectedIndex >= this.diceSets.length
    ) {
    ...
    console.log(
      `You choose the [${this.selectedByUser.faces.join(",")}] dice.`
    );
  ...
  computerSelectsRemainingDice(userIndex: number): number {
    ...
    const random =
      remainingDice[Math.floor(Math.random() * remainingDice.length)];
    this.selectedByComputer = random.dice;

    console.log(
      `I choose the [${this.selectedByComputer.faces.join(",")}] dice.`
    );
    return random.index;
  }
